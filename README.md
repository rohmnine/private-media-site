...
- 图书馆展区：新增 `books.html`，作为与照片 / 视频 / 音频并列的藏品类型，可上传电子书文件（PDF / EPUB / MOBI / AZW3）到 `assets/books`，附书名、作者、简介 / 读后感、展区、标签等元数据，详情页内置在线阅读器：PDF 直接内嵌预览，EPUB 通过本地打包的 epub.js 阅读器分页在线阅读（上一页 / 下一页、左右方向键翻页、阅读进度百分比），其余格式提供下载；所有格式均保留“打开 / 下载电子书”链接。
- 加密相册展柜：照片展区中的指定文件夹可标记为加密，未用查看密码解锁时不会出现在列表、详情、缩略图和 EXIF 接口中。
- 加密图书馆展区：图书馆中的指定展区可在馆长模式下点击展区旁的锁扣标记为加密（复用 `MEDIA_HUB_VIEW_PASSWORD` 查看密码），未解锁时该展区的书籍不会出现在列表、首页预览、时间线、原文件与封面中，进入该展区时会要求输入查看密码解锁。
...
8. 图书馆在 `books.html`；进入馆长模式后，点击展区标签旁的锁扣可将该展区加密 / 取消加密；加密展区需输入 `MEDIA_HUB_VIEW_PASSWORD` 解锁后才能浏览。
